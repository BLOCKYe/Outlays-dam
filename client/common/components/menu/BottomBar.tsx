...
const BottomBar: React.FC<IBottomBarProps> = (props) => {
    ...
    const currentSectionsFactory = (selected: tabsTypes, local: tabsTypes): string => {
        const selectedStyles: string = 'box-content p-2 transition-all bg-w-darker cursor-pointer rounded-full'
        const defaultStyles: string = 'box-content p-2 transition-all cursor-pointer rounded-full text-d-light hover:bg-w-darker hover:text-d'
